...
volatile uint8_t updatedDutycycle = 5U;
...
void LPTMR_HANDLER(void)
{
    ...
    LPTMR_ClearStatusFlags(DEMO_LPTMR_BASE, kLPTMR_TimerCompareFlag);
	  ...
	  if(testtimer >= 1000)
		{
			...
			PRINTF(" 占空比 = %d\r\n",updatedDutycycle);
			PRINTF(" 当前转速 = %d r/s  \r\n",motorcounter/12 );
			PRINTF(" 设置转速 =%d r/s  设置每分钟转速 = %d  \r\n ", target_rpm/60, target_rpm);
			
			current_rpm = motor100ms;
			
      err = current_rpm - target_rpm;   //500 
      ...
      FTM_UpdateChnlEdgeLevelSelect(Board_FTM_2, BOARD_FTM_CHANNEL, 0U);
			
				/* Update PWM duty cycle */
			if (kStatus_Success !=
					FTM_UpdatePwmDutycycle(Board_FTM_2, BOARD_FTM_CHANNEL, kFTM_CenterAlignedPwm, updatedDutycycle))
			{
